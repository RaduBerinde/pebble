...
type Transformer interface {
	...
	Transform(suffixCmp base.CompareRangeKeySuffixes, in Span, out *Span) error
}
...
type TransformerFunc func(base.CompareRangeKeySuffixes, Span, *Span) error

// Transform calls f(cmp, in, out).
func (tf TransformerFunc) Transform(
	suffixCmp base.CompareRangeKeySuffixes, in Span, out *Span,
) error {
	return tf(suffixCmp, in, out)
}

// NoopTransform is a Transformer that performs no mutations.
var NoopTransform Transformer = TransformerFunc(func(_ base.CompareRangeKeySuffixes, s Span, dst *Span) error {
...
func VisibleTransform(snapshot base.SeqNum) Transformer {
	return TransformerFunc(func(_ base.CompareRangeKeySuffixes, s Span, dst *Span) error {
		dst.Start, dst.End = s.Start, s.End
		dst.Keys = dst.Keys[:0]
...
type TransformerIter struct {
	...
	RangeKeySuffixCmp base.CompareRangeKeySuffixes
...
func (t *TransformerIter) applyTransform(span *Span) (*Span, error) {
	...
	if err := t.Transformer.Transform(t.RangeKeySuffixCmp, *span, &t.span); err != nil {
		return nil, err
	}
	return &t.span, nil
}
