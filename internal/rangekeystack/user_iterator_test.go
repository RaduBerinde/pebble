...
func TestIter(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/iter", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "define":
			...
			transform := keyspan.TransformerFunc(func(suffixCmp base.CompareRangeKeySuffixes, s keyspan.Span, dst *keyspan.Span) error {
		...
		b.Run(fmt.Sprintf("shadowing=%t", shadowing), func(b *testing.B) {
			for n := 1; n <= 128; n *= 2 {
				b.Run(fmt.Sprintf("keys=%d", n), func(b *testing.B) {
					...
					for i := 0; i < b.N; i++ {
						err := ui.Transform(testkeys.Comparer.CompareRangeKeySuffixes, keyspan.Span{Keys: keys}, &dst)
						if err != nil {
							b.Fatal(err)
						}
						dst.Keys = dst.Keys[:0]
