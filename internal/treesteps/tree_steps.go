...
func NodeUpdated(n Node) {
	...
	if ns, ok := mu.nodeMap[n]; ok {
		ns.recording.stepLockedf("node %s updated", firstWord(ns.name))
	}
}
...
// Updatef changes the description of the operation and emits a step.
func (op *Op) Updatef(format string, args ...any) {
	...
	mu.Lock()
	defer mu.Unlock()
	op.details = fmt.Sprintf(format, args...)
	op.nodeState.recording.stepLockedf("%s on %s updated", firstWord(op.details), firstWord(op.nodeState.name))
}

// Finishf changes the description of the operation, emits a step, and cleans up
// the operation.
