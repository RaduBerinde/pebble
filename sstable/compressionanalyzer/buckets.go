	...
	for k := BlockKind(0); k < numBlockKinds; k++ {
		for sz := BlockSize(0); sz < numBlockSizes; sz++ {
			for c := Compressibility(0); c < numCompressibility; c++ {
				...
				fmt.Fprintf(tw, "%s\t%s\t%s\t%d\t%s", k, sz, c, bucket.UncompressedSize.Count(), withStdDev(bucket.UncompressedSize, "KB", 1.0/1024))
				for _, e := range (*b)[k][sz][c].Experiments {
					fmt.Fprintf(tw, "\tCR %s", withStdDev(e.CompressionRatio, "", 1.0))
				}
				fmt.Fprintf(tw, "\n")
				fmt.Fprintf(tw, "\t\t\t\t")
				for _, e := range (*b)[k][sz][c].Experiments {
					fmt.Fprintf(tw, "\tComp %s", withStdDev(e.CompressionTime, "us", 1.0))
				}
				fmt.Fprintf(tw, "\n")
				fmt.Fprintf(tw, "\t\t\t\t")
				for _, e := range (*b)[k][sz][c].Experiments {
					fmt.Fprintf(tw, "\tDecomp %s", withStdDev(e.DecompressionTime, "us", 1.0))
				}
				fmt.Fprintf(tw, "\n")
			}
		}
