...
func rewriteKeySuffixesInBlocks(
	...
	for _, c := range w.blockPropCollectors {
		if !c.indexBlock.SupportsSuffixReplacement() {
			return nil, TableFormatUnspecified,
				errors.Errorf("block property collector %s does not support suffix replacement", c.indexBlock.Name())
		}
	}
...
func rewriteDataBlocksToWriter(
	...
	for i := range blocks {
		...
		for i, p := range w.blockPropCollectors {
			if err := p.indexBlock.AddCollectedWithSuffixReplacement(oldProps[i], to); err != nil {
				return err
			}
		}
