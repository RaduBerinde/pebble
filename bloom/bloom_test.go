...
func newTableFilter(bitsPerKey uint32, keys ...[]byte) []byte {
...
`
	...
	require.EqualValues(t, want, filterStr(f))
	...
	for k, want := range m {
		require.EqualValues(t, want, mayContain(f, hash([]byte(k))))
	}
}
...
loop:
	for length := 1; length <= 10000; length = nextLength(length) {
		...
		nFalsePositive := 0
		for i := 0; i < 10000; i++ {
			if mayContain(f, hash(le32(1e9+i))) {
				nFalsePositive++
			}
		}
